...
Public Class liste_eleve_dir
    ...
    Private Sub grdListe_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles grdListe.RowCommand
        ...
        If (e.CommandName = "OpenLg") Then
            ...
            Response.Redirect("~\Fiche_eleve_dir.aspx?id_eleve=" & k)
        End If
        If (e.CommandName = "OpenAb") Then
            Dim index As Integer = Convert.ToInt32(e.CommandArgument)
            Dim k As Integer
            k = grdListe.DataKeys(index).Value.ToString
            Response.Redirect("~\Fiche_eleve_Ab_dir.aspx?id_eleve=" & k)
        End If
        If (e.CommandName = "OpenEC") Then
            Dim index As Integer = Convert.ToInt32(e.CommandArgument)
            Dim k As Integer
            k = grdListe.DataKeys(index).Value.ToString
            Response.Redirect("~\Fiche_eleve_EC_dir.aspx?id_eleve=" & k)
        End If
    End Sub
